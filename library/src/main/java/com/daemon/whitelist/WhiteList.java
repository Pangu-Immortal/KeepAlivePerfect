...
import com.daemon.whitelist.impl.DefaultWhiteListCallback;
import com.daemon.whitelist.impl.DefaultWhiteListProvider;
...
public final class WhiteList {
    ...
    public static List<WhiteListIntentWrapper> getMatchedWhiteListIntent() {
        if (sMatchedWhiteListIntent == null) {
            ...
            List<WhiteListIntentWrapper> intentWrapperList = getAllWhiteListIntent(Daemon.getApplication());
            for (final WhiteListIntentWrapper intentWrapper : intentWrapperList) {
                ...
                if (intentWrapper.getType() == IntentType.DOZE) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        PowerManager pm = (PowerManager) Daemon.getApplication().getSystemService(Context.POWER_SERVICE);
                        if (!pm.isIgnoringBatteryOptimizations(Daemon.getApplication().getPackageName())) {
    ...
    private static void checkCallback(String target) {
        ...
        sIWhiteListCallback.init(target, getApplicationName(Daemon.getApplication()));
    }
